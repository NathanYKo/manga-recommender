        ...
        try:
            ...
            for genre in genres:
                try:
                    ...
                    """
                    execute_query(query, (genre['genre_id'], genre['name']))
                    inserted_count += 1
        ...
        try:
            ...
            for manga in manga_list:
                try:
                    ...
                    # Convert the manga_params dict to a tuple in the correct order
                    params = (
                        manga_params['manga_id'],
                        manga_params['title'],
                        manga_params.get('title_english'),
                        manga_params.get('title_japanese'),
                        manga_params.get('synopsis'),
                        manga_params.get('image_url'),
                        manga_params.get('score'),
                        manga_params.get('scored_by'),
                        manga_params.get('rank'),
                        manga_params.get('popularity'),
                        manga_params.get('status'),
                        manga_params.get('publishing'),
                        manga_params.get('published_from'),
                        manga_params.get('published_to'),
                        manga_params.get('chapters'),
                        manga_params.get('volumes')
                    )
