...
def get_recommender():
    ...
    if recommender is None:
        try:
            ...
            # Create a fallback recommender using just content-based filtering
            try:
                logger.info("Creating fallback content-based recommender...")
                from models.content_based import ContentBasedRecommender
                recommender = ContentBasedRecommender()
                recommender.fit()
                logger.info("Fallback content-based recommender initialized successfully")
            except Exception as e2:
                logger.error(f"Error initializing fallback recommender: {e2}")
                recommender = None
    return recommender
