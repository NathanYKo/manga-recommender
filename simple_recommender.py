...
def get_similar_manga(manga_id, top_n=10):
    ...
    WHERE mg.manga_id = %s
    """
    genre_results = execute_query(genre_query, (manga_id,))
    
    if not genre_results or len(genre_results) == 0 or not genre_results[0]['genres']:
        logger.warning(f"No genres found for manga ID {manga_id}")
        return []
    
    # Get similar manga by matching genres and score
    genres = genre_results[0]['genres'].split(',')
