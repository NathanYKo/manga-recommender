...
def register_routes(app, get_recommender):
    ...
    def manga_details(manga_id):
        ...
        try:
            ...
            if not recommendations.empty:
                ...
                placeholders = ', '.join(['%s'] * len(manga_ids))
                query = f"""
                SELECT m.manga_id, m.title, m.image_url, m.score, string_agg(g.name, ', ') as genres 
                FROM manga m
                LEFT JOIN manga_genres mg ON m.manga_id = mg.manga_id
                LEFT JOIN genres g ON mg.genre_id = g.genre_id
        ...
        if 'user_id' in session:
            ...
            rating_results = execute_query(query, (session['user_id'], manga_id))
            user_rating = rating_results[0]['rating'] if rating_results and len(rating_results) > 0 else None
    ...
    def search():
        ...
        base_query = """
        SELECT m.manga_id, m.title, m.image_url, m.synopsis, m.score, m.popularity, 
               m.published_from, m.status,
        ...
        params = []
        if query_text:
            base_query += " AND (LOWER(m.title) LIKE LOWER(%s) OR LOWER(m.title_english) LIKE LOWER(%s))"
        ...
        if year:
            base_query += " AND EXTRACT(YEAR FROM m.published_from) = %s"
            params.append(int(year))
        
        # Add group by since we're using string_agg
        base_query += """
        GROUP BY m.manga_id
        """
        ...
        if sort_by == 'score':
            ...
            base_query += """
            ORDER BY m.published_from DESC NULLS LAST
            """
        ...
        # Get similar manga for the first result if available
        similar_manga = []
        if results and len(results) > 0:
            try:
                rec_system = get_recommender()
                if rec_system:
                    # Get recommendations for the first manga in the results
                    first_manga_id = results[0]['manga_id']
                    recommendations = rec_system.get_recommendations_by_id(first_manga_id, top_n=6)
                    
                    if not recommendations.empty:
                        manga_ids = recommendations['manga_id'].tolist()
                        placeholders = ', '.join(['%s'] * len(manga_ids))
                        query = f"""
                        SELECT m.manga_id, m.title, m.image_url, m.score, m.synopsis, 
    ...
    def recommend_manga():
        ...
        if request.method == 'POST':
            ...
            # If no title provided, show the form
            if not manga_title:
                flash('Please provide a manga title', 'error')
            ...
            OR LOWER(m.title_english) LIKE LOWER(%s)
            GROUP BY m.manga_id
            ...
            # If we don't have a manga, show an error
            if not results or len(results) == 0:
            ...
            try:
                ...
                if rec_system is None:
                    flash('Recommendation system is currently unavailable. Please try again later.', 'error')
                    logger.error("Recommender system is None, cannot generate recommendations")
                    return render_template('recommend_form.html')
                ...
                if not recommendations.empty:
                    ...
                    # Add similarity scores - handle both hybrid and single-source recommendations
                    similarity_dict = {}
                    for _, row in recommendations.iterrows():
                        if 'hybrid_score' in row:
                            similarity_dict[row['manga_id']] = row['hybrid_score']
                        elif 'similarity' in row:
                            similarity_dict[row['manga_id']] = row['similarity']
                        else:
                            similarity_dict[row['manga_id']] = 0.5  # Default score if none available
    ...
    @app.route('/top-manga')
    @app.route('/top-manga/<int:page>')
    def top_manga(page=1):
        """
        Display a list of top manga sorted by score with pagination.
        Shows 50 manga per page with details like score, rank, and other information.
        """
        per_page = 50
        offset = (page - 1) * per_page
        
        # Count total manga for pagination
        count_query = "SELECT COUNT(*) as total FROM manga WHERE score IS NOT NULL"
        count_result = DatabaseManager.execute_query(count_query)
        total_manga = count_result[0]['total'] if count_result else 0
        
        # Calculate total pages
        total_pages = (total_manga + per_page - 1) // per_page
        
        # Fetch manga with genres, ordered by score
        query = """
        SELECT m.*, 
               string_agg(g.name, ', ') as genres,
        ...
        results = DatabaseManager.execute_query(query, (per_page, offset))
        standardize_manga_fields(results)
    ...
    def search_suggestions():
        """API endpoint to provide search suggestions as user types in search bar."""
        query_text = request.args.get('q', '')
        logger.info(f"Search suggestions request: q={query_text}")
        
        if not query_text or len(query_text) < 2:
            logger.info("Search query too short, returning empty results")
            return jsonify([])
            
        # Query database for manga titles that match the search query
        query = """
        SELECT manga_id, title, COALESCE(title_english, '') as title_english, 
               image_url, score
        FROM manga 
        ...
        try:
            results = execute_query(query, (f'%{query_text}%', f'%{query_text}%'))
            ...
            logger.info(f"Found {len(suggestions)} manga suggestions for '{query_text}'")
