...
class HybridRecommender:
    ...
    def get_similar_manga(self, manga_id, top_n=6):
        """Get similar manga based on hybrid model."""
        try:
            # Check if manga exists
            manga_query = "SELECT title FROM manga WHERE manga_id = %s"
            from utils.helpers import execute_query
            manga_result = execute_query(manga_query, (manga_id,), fetch_one=True)
            
            if not manga_result:
                logger.warning(f"Manga ID {manga_id} not found in database")
                return pd.DataFrame(columns=['manga_id', 'title', 'hybrid_score'])
            
            # Get content-based recommendations
            content_recs = self.content_based.get_recommendations_by_id(manga_id, top_n=top_n*2)
            
            # Get collaborative filtering recommendations
            collab_recs = self.collaborative.get_recommendations_by_id(manga_id, top_n=top_n*2)
            ...
            # Scale similarity scores to 0-1 range for each method
            content_max = content_recs['similarity'].max()
            collab_max = collab_recs['similarity'].max()
            
            content_recs['normalized_similarity'] = content_recs['similarity'] / content_max if content_max > 0 else 0
            collab_recs['normalized_similarity'] = collab_recs['similarity'] / collab_max if collab_max > 0 else 0
            
            # Merge recommendations
            content_recs = content_recs[['manga_id', 'title', 'normalized_similarity']]
            content_recs.columns = ['manga_id', 'title', 'content_similarity']
            
            collab_recs = collab_recs[['manga_id', 'normalized_similarity']]
            collab_recs.columns = ['manga_id', 'collab_similarity']
            ...
            return hybrid_recommendations[['manga_id', 'title', 'hybrid_score']].head(top_n)
        
        except Exception as e:
            logger.error(f"Error in get_similar_manga: {e}")
            return pd.DataFrame(columns=['manga_id', 'title', 'hybrid_score'])
