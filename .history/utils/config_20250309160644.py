...
class Config:
    ...
    def get_db_config(self):
        ...
        if database_url:
            ...
            # Parse the URL format: postgresql://username:password@host:port/dbname
            if '@' in database_url:
                auth, conn_info = database_url.split('@', 1)
                ...
                if '/' in conn_info:
                    host_port, dbname = conn_info.split('/', 1)
            ...
            logger = logging.getLogger('manga_recommender.config')
            logger.info(f"Using database config from DATABASE_URL: host={db_config.get('host')}, user={db_config.get('user')}, database={db_config.get('database')}")
