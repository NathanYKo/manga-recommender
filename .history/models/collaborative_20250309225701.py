...
class CollaborativeFilteringRecommender:
        ...
        if len(self.ratings_df) < 100:
            ...
            for _, manga in self.manga_df.iterrows():
                if manga['score'] is not None:
                    ...
                    popularity = 0
                    if 'popularity' in manga and manga['popularity'] is not None:
                        try:
                            popularity = float(manga['popularity'])
                        except (ValueError, TypeError):
                            popularity = 0
                    
                    # Ensure we have at least 5 ratings per manga, regardless of popularity
                    num_ratings = min(len(user_ids), max(5, int(popularity / 100) if popularity else 5))
    ...
    def fit(self, k=10, metric='cosine', algorithm='brute'):
        ...
        try:
            ...
            if self.manga_user_matrix is None or self.manga_user_matrix.shape[0] == 0:
            ...
            logger.error(f"Error fitting collaborative model: {str(e)}")
            # Ensure model and matrices are reset if there's an error
            self.model = None
            self.manga_user_matrix = None
            self.user_manga_matrix = None
            self.manga_indices = {}
